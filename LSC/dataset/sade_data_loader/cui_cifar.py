...
cifar100_json_path_dict = {
    0.01: '../../LT/NCL/dataset_json/cifar-100-data/cifar100_imbalance100',
    0.02: '../../LT/NCL/dataset_json/cifar-100-data/cifar100_imbalance50'
}

class CIFAR100_BASE(BaseSet):
    def __init__(self, mode='train', transform=None, imb=0.01):
        json_path = os.path.join(cifar100_json_path_dict[imb], f'cifar100_imbalance100_{mode}.json')
        super().__init__(mode, json_path=json_path, transform=transform)
        self.class_dict = self._get_class_dict()
    ...
    def collect_labels(self, ):
        num_cls_list = [0] * 100
        targets = []
        for now_info in self.data:
            now_label = now_info['category_id']
            num_cls_list[now_label] += 1
            targets.append(now_label)

        self.targets = targets

        return targets, num_cls_list
    ...
    def __getitem__(self, index):
        ...
        image = self.transform(img_org)
        if self.targets is None:
            self.collect_labels()
        image_label = self.targets[index]
