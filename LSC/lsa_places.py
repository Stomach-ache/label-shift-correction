...
from dataset.sade_ds import Places_LT
from dataset.cifar_c_dataset import distrb_dict
...
from utils.utils import get_num_cls_list_from_label
...
class LDDateset(Dataset):
    def __init__(self, feat: torch.Tensor, label: torch.Tensor, imb_ub=50.0, step=.1, load_path=None, clc_num=100):
        ...
        if load_path is None or not osp.exists(osp.join(load_path, 'distri_list')):
            ...
            f1_flag = True
            f2_flag = True
            # idx_each_class_tensor = torch.stack(idx_each_class)
            while cur_imb < imb_ub:
                # print(f"current imb: {cur_imb}")
                img_num_per_cls1 = self.set_imbalance_data(img_max, cur_imb, False, self.clc_num)  # 头部少
                img_num_per_cls2 = [i for i in reversed(img_num_per_cls1)]  # 头部多
...
def proc_ide_logits_list(train_logits, label, top_k='ada', tol=0.9):
    ...
    for i in range(3):
        ...
        # prob_item = F.softmax(logits_item, dim=1)
...
def get_dataset_wrap(feat_dir, top_k, save_data_path=None, tol=.95, force_gen_data=True, step=.1):
    load_root_dir_list = [
        feat_dir,
        # 'output/feat/imagenet_ncl_base/augmix_121_22',
        # 'output/feat/imagenet_ncl_augmix/randaug_seed0',
        ]

    ide_logits, label = proc_logtis_and_label_list(load_root_dir_list, top_k, tol)
    # draw_train_logtis(ide_logits, label)
    # exit()
    if osp.exists(osp.join(save_data_path, 'distri_list')) and not force_gen_data:
        ide_train_set = LDDateset(ide_logits, label, imb_ub=100.0, step=step, load_path=save_data_path, clc_num=class_num)
    else:
        ide_train_set = LDDateset(ide_logits, label, imb_ub=100.0, step=step, clc_num=class_num)  # TODO 设置为 200
        ide_train_set.save_param(save_data_path)
    # ide_train_set.draw_distribution(0)
    # ide_train_set.draw_distribution(1900)
    # ide_train_set.draw_distribution(1901)
    # exit()
...
def run_one(seed, feat_dir, top_k='ada', num_epoch=100, tol=0.95, force_gen_data=True, step=.1):
    ...
    # plot_bar_chart(ide_train_set.distri_targets[500][lbl_freq_list]*200, ide_train_set.distri_list[500].reshape(3, -1).sum(dim=0)[lbl_freq_list], 'lddataset')  # TODO
    ide_loader = DataLoader(ide_train_set, batch_size=128, shuffle=True)  # batchsize 对 MLP 的学习也有比较大的影响
    ...
    # bsce_weight = torch.tensor(ide_train_set.num_cls_list) / 50
    tot_loss = 0
    for epoch in range(num_epoch):
        ...
        for feat, label in ide_loader:
            ...
            # predicted = output / output.sum(dim=1, keepdim=True)
            # bs_output = output + bsce_weight.log()
            # compensate logits
            # la_logits = feat.view(-1, 3, 100) + output.view(-1, 1, 100).log()
            # la_logits_avg = la_logits.mean(dim=1)
            loss = F.kl_div(F.log_softmax(output, dim=1), label, reduction='batchmean')
            # loss = F.cross_entropy(output, label)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            # ide_model.max_norm_reg.PGD(ide_model.pgd_param)
            tot_loss += loss
...
def LDE_adapt(net, lde_model, val_dataset, val_loader, org_logits_path, force_pred_logits=False):
    ...
    PDA_model = PDA(net, optimizer=optimizer, class_num=class_num)
    ...
    if dist_flag == 1:
        ...
        # infer_distri = F.softmax(final_tensor.mean(dim=0).cuda(), dim=0)
    elif dist_flag == 0:
...
def get_places_lt_args():
    ...
    parser.add_argument('--tol', default=0.95, type=float, help='tolerance of the threshold ')
    ...
    parser.add_argument('--model_dir', default='pre_trained/places/NCL_places.pth', help='dir of pre-trained model')
    parser.add_argument('--data_dir', default='../data/Places365/places365_standard', help='path of dataset')
...
def main():
    ...
    net = model_load.load_ncl_places_wo_con(model_dir)
    net = net.cuda()
    result = []
    record_str = ""
    for distri in used_distri:
        ...
        test_txt = f'./data_txt/Places_LT_v2/Places_LT_{distri}.txt'
        print(f"now dist {distri}")
        for s in seedlist:
            places_data_loader = Places_LT(
                data_dir,
                batch_size=128,
                training=False,
                num_workers=8,
            ...
            val_set = places_data_loader.val_dataset
            val_set.num_per_cls_list = val_set.cls_num_list #
            val_loader = places_data_loader.test_set()
            train_set = places_data_loader.trained_dataset
            ...
            if osp.exists(model_dir) and not force_train_one:
                ...
                LDE_model = run_one(s, feat_dir=feat_dir, top_k=topk, num_epoch=num_epoch, tol=tol, force_gen_data=force_gen_data, step=step)
            ...
            if os.path.exists(org_logits_path):
                ...
                org_logits = None
            net.div_out = False
            sum_org_logits = org_logits.reshape((-1, 3, class_num)).sum(dim=1).cuda()
            use_logits = sum_org_logits + PDA_model.bsce_weight.log() * PDA_model.temper

            # use_logits = None
            top1, top5, *freq = validate(val_loader, PDA_model, None, name_dataset, gpu=0, print_freq=10, debug=False,
